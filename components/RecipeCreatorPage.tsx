...
import { ArrowLeft, Plus, X, Camera, Clock, Users, ChefHat, Star } from 'lucide-react';
...
type CreatorStep = 'basic' | 'ingredients' | 'instructions' | 'nutrition' | 'preview';

export function RecipeCreatorPage({ onNavigateBack, onSaveRecipe }: RecipeCreatorPageProps) {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 'basic':
        return <BasicInfoStep recipe={recipe} setRecipe={setRecipe} addTag={addTag} removeTag={removeTag} />;
      case 'ingredients':
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center space-y-1 p-2 rounded-lg transition-colors ${
                isActive ? 'text-[#F14C35]' : isCompleted ? 'text-green-600' : 'text-gray-400'
              }`}
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                isActive ? 'bg-[#F14C35]/20' : isCompleted ? 'bg-green-100' : 'bg-gray-100'
              }`}>
...
// Step Components
function BasicInfoStep({ recipe, setRecipe, addTag, removeTag }: {
  recipe: NewRecipe;
  ...
  const cuisines = ['Italian', 'Chinese', 'Japanese', 'Mexican', 'Indian', 'Thai', 'French', 'American', 'Mediterranean'];

  return (
    <div className="space-y-6 pb-24">
      <div>
        <h2 className="text-xl font-semibold text-[#0B1F3A] mb-6">Recipe Basics</h2>
        
        {/* Photo Upload */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-[#0B1F3A] mb-3">Recipe Photo</label>
          <div className="w-full h-48 bg-gray-100 rounded-xl flex items-center justify-center border-2 border-dashed border-gray-300 hover:border-[#F14C35] transition-colors cursor-pointer">
            {recipe.mainImage ? (
              <ImageWithFallback src={recipe.mainImage} alt="Recipe" className="w-full h-full object-cover rounded-xl" />
            ) : (
              <div className="text-center">
        ...
        {/* Title */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Recipe Title</label>
          <input
            type="text"
            value={recipe.title}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Difficulty</label>
            <select
        ...
        {/* Cuisine */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Cuisine Type</label>
          <select
        ...
        {/* Tags */}
        <div>
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Tags</label>
...
function IngredientsStep({ ingredients, addIngredient, updateIngredient, removeIngredient }: {
  ingredients: RecipeIngredient[];
  addIngredient: () => void;
  updateIngredient: (id: string, field: keyof RecipeIngredient, value: string) => void;
  removeIngredient: (id: string) => void;
}) {
  const units = ['cups', 'tbsp', 'tsp', 'oz', 'lbs', 'grams', 'kg', 'ml', 'liters', 'pieces', 'cloves', 'pinch'];
          ...
          {ingredients.map((ingredient, index) => (
            <div key={ingredient.id} className="bg-[#F8F9FA] rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="font-medium text-[#0B1F3A]">Ingredient {index + 1}</span>
                <button
...
function InstructionsStep({ steps, addStep, updateStep, removeStep }: {
  steps: RecipeStep[];
  addStep: () => void;
  updateStep: (id: string, field: keyof RecipeStep, value: string | number) => void;
        ...
        <div className="space-y-4">
          {steps.map((step) => (
            <div key={step.id} className="bg-[#F8F9FA] rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="font-medium text-[#0B1F3A]">Step {step.stepNumber}</span>
                <button
...
function NutritionStep({ recipe, setRecipe }: {
  recipe: NewRecipe;
  setRecipe: React.Dispatch<React.SetStateAction<NewRecipe>>;
}) {
  return (
    <div className="space-y-6 pb-24">
      <h2 className="text-xl font-semibold text-[#0B1F3A]">Nutrition Information</h2>
      <p className="text-gray-600">Add optional nutrition info per serving</p>
      
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Calories</label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Protein (g)</label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Carbs (g)</label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Fat (g)</label>
          <input
            type="number"
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        {/* Header */}
        <div className="p-6">
          <h3 className="text-2xl font-bold text-[#0B1F3A] mb-2">{recipe.title}</h3>
