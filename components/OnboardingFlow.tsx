...
import { FuzoInput } from './FuzoInput';
...
type OnboardingStep = 'splash' | 'auth' | 'profile' | 'cuisine' | 'dietary' | 'location' | 'notifications' | 'success';
...
export function OnboardingFlow({ onComplete, onBack }: OnboardingFlowProps) {
  ...
  const handleAuth = async () => {
    ...
    if (isLogin) {
      // Login flow
      if (userData.email === DUMMY_CREDENTIALS.email && userData.password === DUMMY_CREDENTIALS.password) {
  ...
  const nextStep = () => {
    const steps: OnboardingStep[] = ['splash', 'auth', 'profile', 'cuisine', 'dietary', 'location', 'notifications', 'success'];
  ...
  const renderTakoMascot = (pose: 'waving' | 'map' | 'bell' | 'celebration') => {
    const takoEmojis = {
      waving: '👋',
      map: '🗺️',
      bell: '🔔',
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden relative">
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              <div className="w-32 h-32 mx-auto mb-8 bg-white/10 backdrop-blur rounded-2xl flex items-center justify-center border border-white/20">
                <span className="text-5xl font-bold text-primary-foreground">F</span>
              </div>

              <div className="mb-8">
                <h1 className="text-4xl font-bold tracking-tight mb-4">FUZO</h1>
                <p className="text-xl text-white/90">Discover food, near &amp; far.</p>
              </div>
          ...
          >
            <div style={{ maxWidth: '448px', margin: '0 auto' }}>
              ...
              <div style={{ 
                ...
                >
                  <ArrowLeft style={{ width: '20px', height: '20px', color: 'var(--fuzo-navy)' }} />
                </button>
                <div style={{ 
                  width: '48px', 
                  height: '48px', 
              ...
              <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                ...
                <p style={{ color: '#6B7280' }}>Welcome to your food adventure</p>
                {isLogin && (
                  <div style={{ 
                    marginTop: '16px', 
                    padding: '12px', 
                ...
                <>
                  ...
                  <div style={{ 
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        backgroundColor: !isLogin ? 'var(--fuzo-white)' : 'transparent',
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    <button
                      ...
                      onMouseLeave={(e) => {
                        if (!loading && !isSigningIn) {
                          (e.target as HTMLButtonElement).style.backgroundColor = 'var(--fuzo-white)';
                        }
                      }}
                    >
                  ...
                  <div style={{ 
                    ...
                    <div style={{ 
                      ...
                      <span style={{ 
                        ...
                        fontSize: '0.875rem', 
                        color: '#6B7280' 
                      }}>Or {isLogin ? 'login' : 'sign up'} with email</span>
                    </div>
                  </div>
          ...
          >
            <div style={{ maxWidth: '448px', margin: '0 auto' }}>
              ...
              <div className="text-center mb-8">
                ...
                <button className="text-[#F14C35] font-medium">Upload photo</button>
              </div>
              ...
              <FuzoButton variant="primary" size="lg" className="rounded-lg max-w-md w-full mx-auto" onClick={nextStep}>
                Continue
              </FuzoButton>
            </div>
          </motion.div>
        ...
        {currentStep === 'cuisine' && (
          <motion.div key="cuisine" {...pageTransition} className="min-h-screen bg-white px-6 py-8 flex flex-col">
            <div className="max-w-md mx-auto flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <button onClick={prevStep} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
                  <ArrowLeft className="w-5 h-5 text-[#0B1F3A]" />
                </button>
                <h1 className="font-bold text-[#0B1F3A]">Cuisine Preferences</h1>
                <div className="w-10"></div>
              </div>

              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-[#0B1F3A] mb-2">What flavors call to you?</h2>
                <p className="text-gray-600">Choose your favorite cuisines (select multiple)</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-8 flex-1">
                {cuisineOptions.map((cuisine) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-[#0B1F3A] text-sm text-center">{cuisine.name}</span>
                    {userData.cuisines.includes(cuisine.id) && (
              ...
              <FuzoButton variant="primary" size="lg" className="w-full" onClick={nextStep}>
                Next
              </FuzoButton>
            </div>
          </motion.div>
        ...
        {currentStep === 'dietary' && (
          <motion.div key="dietary" {...pageTransition} className="min-h-screen bg-white px-6 py-8 flex flex-col">
            <div className="max-w-md mx-auto flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <button onClick={prevStep} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
                  <ArrowLeft className="w-5 h-5 text-[#0B1F3A]" />
                </button>
                <h1 className="font-bold text-[#0B1F3A]">Dietary Preferences</h1>
                <div className="w-10"></div>
              </div>

              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-[#0B1F3A] mb-2">Any dietary needs?</h2>
                <p className="text-gray-600">Help us show you the right food options</p>
              </div>
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          userData.dietary.includes(option.id) ? 'bg-[#F14C35]' : 'bg-gray-100'
                        }`}>
                          <Icon className={`w-5 h-5 ${
                            userData.dietary.includes(option.id) ? 'text-white' : 'text-gray-600'
                          }`} />
                        </div>
                        <span className="font-medium text-[#0B1F3A]">{option.name}</span>
                      </div>
                      {userData.dietary.includes(option.id) && (
                        <Check className="w-5 h-5 text-[#F14C35]" />
                      )}
        ...
        {currentStep === 'location' && (
          <motion.div key="location" {...pageTransition} className="min-h-screen bg-white px-6 py-8 flex flex-col">
            <div className="max-w-md mx-auto flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <button onClick={prevStep} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
                  <ArrowLeft className="w-5 h-5 text-[#0B1F3A]" />
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
                ...
                <div className="space-y-4">
                  <h2 className="text-2xl font-bold text-[#0B1F3A]">Enable location to discover food near you</h2>
                  <p className="text-gray-600">We'll show you restaurants, cafes, and food spots in your area</p>
                </div>
                ...
                <div className="w-full space-y-4">
                  <FuzoButton 
                    ...
                    className="w-full"
                    onClick={() => {
                      setUserData(prev => ({ ...prev, locationEnabled: true }));
                      nextStep();
                    }}
                  >
                  ...
                  <FuzoButton 
                    ...
                    className="w-full"
                    onClick={() => {
                      setUserData(prev => ({ ...prev, locationEnabled: false }));
                      nextStep();
                    }}
                  >
                    Not Now
        ...
        {currentStep === 'notifications' && (
          <motion.div key="notifications" {...pageTransition} className="min-h-screen bg-white px-6 py-8 flex flex-col">
            <div className="max-w-md mx-auto flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <button onClick={prevStep} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
                  <ArrowLeft className="w-5 h-5 text-[#0B1F3A]" />
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
                ...
                <div className="space-y-4">
                  <h2 className="text-2xl font-bold text-[#0B1F3A]">Get notified about food adventures</h2>
                  <p className="text-gray-600">New restaurants, friend activity, and personalized recommendations</p>
                </div>
                ...
                <div className="w-full space-y-4">
                  <FuzoButton 
                    ...
                    className="w-full"
                    onClick={() => {
                      setUserData(prev => ({ ...prev, notificationsEnabled: true }));
                      nextStep();
                    }}
                  >
                  ...
                  <FuzoButton 
                    ...
                    className="w-full"
                    onClick={() => {
                      setUserData(prev => ({ ...prev, notificationsEnabled: false }));
                      nextStep();
                    }}
                  >
                    Maybe Later
        ...
        {currentStep === 'success' && (
          <motion.div 
            ...
            style={{
              minHeight: '100vh',
              background: 'linear-gradient(135deg, var(--fuzo-coral) 0%, var(--fuzo-orange-brown) 100%)',
              display: 'flex',
          ...
          >
            ...
            >
              ...
              <div style={{ margin: '32px 0' }}>
                ...
                }}>Your food adventure begins now</p>
              </div>
              ...
              >
                <p style={{ 
                  ...
                  marginBottom: '16px' 
                }}>🎉 Setup complete! You're ready to:</p>
                <div style={{ textAlign: 'left' }}>
                  <p style={{ 
                    color: 'rgba(255, 255, 255, 0.9)', 
                    marginBottom: '8px' 
                  }}>• Swipe through amazing restaurants</p>
                  <p style={{ 
                    color: 'rgba(255, 255, 255, 0.9)', 
                    marginBottom: '8px' 
                  }}>• Connect with food-loving friends</p>
                  <p style={{ 
                    color: 'rgba(255, 255, 255, 0.9)', 
                    marginBottom: '8px' 
                  }}>• Capture and share your food moments</p>
                  <p style={{ 
                    color: 'rgba(255, 255, 255, 0.9)' 
                  }}>• Discover new recipes and flavors</p>
                </div>
              </motion.div>
