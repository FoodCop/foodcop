...
import { MapPin, Plus, Minus, Navigation, Layers } from 'lucide-react';
import { API_CONFIG } from './config/apiConfig';
import { backendService } from './services/backendService';
import { mapLibreLocationService, LocationData } from './services/mapLibreLocationService';
...
export function MapView({ restaurants, selectedRestaurant, onRestaurantSelect }: MapViewProps) {
  ...
  const [centerCoords, setCenterCoords] = useState({ lat: 37.7749, lng: -122.4194 });
  ...
  const initializeMapLibre = async () => {
    ...
    try {
      ...
      map.on('error', (e) => {
        ...
        const isTileError = e.error?.message?.includes('Failed to fetch') && e.sourceId;
        const isPermissionError = e.error?.message?.includes('403') || e.error?.message?.includes('Forbidden');
      ...
      try {
        ...
        markerElement.innerHTML = `
          <div class="relative w-8 h-8 rounded-full border-3 border-white shadow-lg cursor-pointer transform transition-transform hover:scale-110" 
               style="background-color: ${restaurant.isSaved ? '#F14C35' : '#A6471E'}">
  ...
  const showPopup = (restaurant: any) => {
    ...
    const popupContent = `
      <div class="p-3 max-w-xs bg-white rounded-lg shadow-lg">
        <img src="${restaurant.image}" alt="${restaurant.name}" class="w-full h-24 object-cover rounded-lg mb-2" loading="lazy">
        <h3 class="font-bold text-[#0B1F3A] text-sm mb-1">${restaurant.name}</h3>
        ...
        <div class="flex flex-wrap gap-1">
          ${restaurant.cuisine.map(c => `<span class="px-2 py-1 bg-[#F14C35]/10 text-[#F14C35] text-xs rounded-full">${c}</span>`).join('')}
        </div>
      </div>
    `;
  ...
  useEffect(() => {
    if (selectedRestaurant && mapLibreRef.current) {
      mapLibreRef.current.flyTo({
        center: [selectedRestaurant.coordinates.lng, selectedRestaurant.coordinates.lat],
        zoom: 16,
        duration: 1000
      });
  ...
  useEffect(() => {
    if (mapLibreRef.current) {
      ...
      if (userLocationMarkerRef.current) {
        userLocationMarkerRef.current.setLngLat([centerCoords.lng, centerCoords.lat]);
      }
  ...
  const handleMyLocation = () => {
    if (!navigator.geolocation) {
      ...
      if (mapLibreRef.current) {
        mapLibreRef.current.flyTo({
          center: [API_CONFIG.DEFAULT_LOCATION.lng, API_CONFIG.DEFAULT_LOCATION.lat],
          zoom: 14
        });
      }
      return;
      ...
      (error) => {
        ...
        switch (error.code) {
          ...
          case error.TIMEOUT:
            ...
            break;
          default:
            errorMessage = `Map location error: ${error.message || 'Unknown error'}`;
        }
  ...
  useEffect(() => {
    if (mapLibreRef.current && mapLoaded) {
      try {
        ...
        mapLibreRef.current.setStyle(newStyle);
      } catch (error) {
