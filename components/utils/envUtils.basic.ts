...
function safeEnvAccess(key: string): string | undefined {
  try {
    ...
    if (import.meta?.env && key in import.meta.env && import.meta.env[key]) {
      console.debug(`[env] Found ${key} via import.meta.env`);
      return import.meta.env[key];
    }
  } catch (error) {
    console.warn(`[env] Error accessing import.meta.env.${key}:`, error);
  }

  try {
    // Try process.env
    if (typeof process !== 'undefined' && process.env && key in process.env && process.env[key]) {
      console.debug(`[env] Found ${key} via process.env`);
      return process.env[key];
    }
  } catch (error) {
    console.warn(`[env] Error accessing process.env.${key}:`, error);
  }

  try {
    // Try window object for client-side (sometimes Vite injects here)
    if (typeof window !== 'undefined' && (window as any).__VITE_ENV__?.[key]) {
      console.debug(`[env] Found ${key} via window.__VITE_ENV__`);
      return (window as any).__VITE_ENV__[key];
    }
  } catch (error) {
    console.warn(`[env] Error accessing window.__VITE_ENV__.${key}:`, error);
  }

  console.warn(`[env] Environment variable ${key} not found in any source`);
  return undefined;
}
...
export const logEnvStatus = (): void => {
  try {
    ...
    console.log('[env] FUZO Environment Status:');
    ...
    // Debug info
    console.log('[env] Environment Debug:');
    ...
    console.error('[env] Error in environment status logging:', error);
  }
};
