import { APP_ENV, FEATURES, getEnv } from '../../utils/env';
...
export function getGoogleApiKeySafe(): string | null {
  ...
  console.log('🔑 API Key Validation:', {
    ...
    value: key ? (key.length > 10 ? `${key.substring(0, 8)}...` : key) : 'missing',
  ...
  if (!key.startsWith('AIza')) {
    console.warn('⚠️ Google Maps API key format appears invalid - expected to start with "AIza"');
    console.log('💡 If this is a valid key, it might work anyway. Proceeding...');
...
export const hasRealAPIsConfigured = (): boolean => {
  return isGoogleAPIAvailable() || isStreamChatAvailable() || isAIAssistantAvailable();
};
...
export const buildDistanceMatrixURL = (origins: string[], destinations: string[], params: Record<string, any> = {}) => {
  const apiKey = getGoogleApiKeySafe();
  if (!apiKey) {
    throw new Error('Google API key not available');
  }
...
export const buildDirectionsURL = (origin: string, destination: string, params: Record<string, any> = {}) => {
  const apiKey = getGoogleApiKeySafe();
  if (!apiKey) {
    throw new Error('Google API key not available');
  }
...
export const buildPlaceDetailsURL = (placeId: string, params: Record<string, any> = {}) => {
  const apiKey = getGoogleApiKeySafe();
  if (!apiKey) {
    throw new Error('Google API key not available');
  }
...
// Google Photos API
export const buildPhotoURL = (photoReference: string, maxWidth: number = 400, maxHeight?: number) => {
  const apiKey = getGoogleApiKeySafe();
  if (!apiKey) {
    throw new Error('Google API key not available');
  }
