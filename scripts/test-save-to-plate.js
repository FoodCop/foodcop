...
// Test SaveToPlate functionality
async function testSaveToPlate() {
  console.log("🧪 Testing SaveToPlate functionality...");
  ...
  try {
    ...
    // Test saving a restaurant item
    const testItem = {
      plate_id: user.id,
      item_type: "restaurant",
      item_id: "test-restaurant-123",
      metadata: {
    ...
    // Test idempotency - try to save the same item again
    ...
    if (duplicateError) {
      if (duplicateError.code === "23505") {
        console.log(
          "✅ Idempotency working - duplicate prevented (unique constraint)"
        );
      } else {
        console.error(
          "❌ Unexpected error on duplicate:",
          duplicateError.message
        );
      }
    } else {
      console.log("⚠️  Duplicate was allowed (this might be expected)");
    }
...
// Run the test
testSaveToPlate().catch(console.error);
