// Seed Master Bot users in the users table
// Run with: node scripts/seed-masterbot-users.js
...
// The 7 AI connoisseurs with their actual user IDs from master_bots table
...
async function seedMasterBotUsers() {
  ...
  try {
    ...
    const { data: existingUsers, error: checkError } = await supabase
      .from("users")
      .select("id, username")
      .in("id", masterBotUsers.map((user) => user.id));
    ...
    if (existingUsers && existingUsers.length > 0) {
      console.log("⚠️  Some Master Bot users already exist:");
    ...
    const { data, error } = await supabase
      .from("users")
      .upsert(masterBotUsers, { onConflict: "id" });
    ...
    console.log("✅ Successfully seeded Master Bot users!");
    console.log(`   - Added/Updated ${masterBotUsers.length} users`);
