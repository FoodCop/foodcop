...
test.describe("User Profile and Plates Functionality", () => {
  ...
  test("should create user profile and save restaurant to plate", async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="Bio"]', "Food lover and restaurant explorer");
    ...
    await expect(page.locator('text=Restaurant saved to your plate')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="saved-restaurant"]')).toBeVisible();
    
    // Verify restaurant details
    const savedRestaurant = page.locator('[data-testid="saved-restaurant"]').first();
  ...
  test("should save photo to user profile", async ({ page }) => {
    ...
    await expect(savedPhoto.locator('text=pasta, italian, dinner')).toBeVisible();
  });
  ...
  test("should update user profile information", async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="Bio"]', "Updated bio - food enthusiast");
    ...
    await expect(page.locator('text=Updated bio - food enthusiast')).toBeVisible();
  ...
  test("should handle plate management (save/unsave restaurants)", async ({ page }) => {
    ...
    const secondRestaurant = page.locator('[data-testid="restaurant-card"]').nth(1);
    ...
    await expect(page.locator('[data-testid="saved-restaurant"]')).toHaveCount(2);
    
    // Step 4: Unsave one restaurant
    const firstSavedRestaurant = page.locator('[data-testid="saved-restaurant"]').first();
    ...
    await expect(page.locator('[data-testid="saved-restaurant"]')).toHaveCount(1);
  });
  ...
  test("should persist user data across sessions", async ({ page, context }) => {
    ...
    if (await page.locator('input[placeholder*="Display Name"]').isVisible()) {
      await page.fill('input[placeholder*="Display Name"]', "Persistence Test User");
      await page.fill('textarea[placeholder*="Bio"]', "Testing data persistence");
      await page.click('button:has-text("Complete Profile")');
      await page.waitForURL("**/feed");
    }
    ...
    await expect(newPage.locator('text=Testing data persistence')).toBeVisible();
    
    // Verify saved restaurant
    await newPage.click('button:has-text("Plates")');
    await expect(newPage.locator('[data-testid="saved-restaurant"]')).toBeVisible();
  });

  test("should handle profile loading states and errors gracefully", async ({ page }) => {
    // Test loading state
    await page.goto("http://localhost:5173/profile");
    ...
    await page.waitForLoadState("networkidle");
    await expect(page.locator('[data-testid="profile-loading"]')).not.toBeVisible();
    ...
    if (await page.locator('[data-testid="restaurant-card"]').isVisible()) {
      ...
      await expect(page.locator('text=Failed to save restaurant')).toBeVisible();
    }
  });
});
