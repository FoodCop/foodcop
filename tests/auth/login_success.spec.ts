...
test.describe('OAuth Login Success Flow', () => {
  ...
  test('should handle OAuth callback with valid session', async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/auth/callback?code=valid_code&state=valid_state');
  ...
  test('should clean up URL parameters after OAuth callback', async ({ page }) => {
    // Navigate to callback with parameters
    await page.goto('http://localhost:5173/auth/callback?code=test&state=test&onboarding=auth');
