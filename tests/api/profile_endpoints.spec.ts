...
test.describe("Profile API Endpoints", () => {
  ...
  test("should save restaurant to user plate", async ({ request }) => {
    ...
    const saveResponse = await request.post(`${API_BASE}/profile/save-restaurant`, {
      headers: {
        'Authorization': `Bearer ${authToken}`,
        'Content-Type': 'application/json'
      },
  ...
  test("should retrieve saved restaurants", async ({ request }) => {
    ...
    const getResponse = await request.get(`${API_BASE}/profile/saved-restaurants`, {
      headers: {
        'Authorization': `Bearer ${authToken}`
      }
    });
  ...
  test("should handle duplicate restaurant saves", async ({ request }) => {
    ...
    const firstSave = await request.post(`${API_BASE}/profile/save-restaurant`, {
      headers: {
        'Authorization': `Bearer ${authToken}`,
        'Content-Type': 'application/json'
      },
    ...
    const secondSave = await request.post(`${API_BASE}/profile/save-restaurant`, {
      headers: {
        'Authorization': `Bearer ${authToken}`,
        'Content-Type': 'application/json'
      },
  ...
  test("should remove restaurant from plate", async ({ request }) => {
    ...
    // Then remove it
    const removeResponse = await request.delete(`${API_BASE}/profile/saved-restaurants/${restaurantData.restaurant.place_id}`, {
      headers: {
        'Authorization': `Bearer ${authToken}`
      }
    });
