...
describe("FuzoCard", () => {
  ...
  it("renders with different backgrounds", () => {
    const { rerender } = render(
      <FuzoCard background="yellow">Yellow card</FuzoCard>
    );
    expect(screen.getByText("Yellow card")).toHaveClass(
      "bg-[#FFD74A]",
      "text-[#0B1F3A]"
    );

    rerender(<FuzoCard background="coral">Coral card</FuzoCard>);
    expect(screen.getByText("Coral card")).toHaveClass(
      "bg-[#F14C35]",
      "text-white"
    );

    rerender(<FuzoCard background="dark">Dark card</FuzoCard>);
    expect(screen.getByText("Dark card")).toHaveClass(
      "bg-[#A6471E]",
      "text-white"
    );
  });
  ...
  it("renders with different padding sizes", () => {
    const { rerender } = render(
      <FuzoCard padding="sm">Small padding</FuzoCard>
    );
