...
// Cache for image metadata (only _v2 images)
let imageMetadataCache: Map<string, string> | null = null;
let imageMetadataLoading: Promise<void> | null = null;

/**
 * Load image metadata from JSON file and create a lookup map by placeId
 * Only includes images with _v2 suffix
 */
async function loadImageMetadata(): Promise<Map<string, string>> {
  ...
  if (imageMetadataLoading) {
    await imageMetadataLoading;
    return imageMetadataCache!;
  }

  // Start loading
  imageMetadataLoading = (async () => {
    try {
      console.log('🖼️ Loading image metadata from generated-images...');
      const response = await fetch('/generated-images/image-metadata.json');
      ...
      // Create lookup map: placeId -> imagePath (only _v2 images)
      const lookupMap = new Map<string, string>();

      for (const item of metadata) {
        // Only include images with _v2 suffix
        if (item.imagePath.includes('_v2.png')) {
          lookupMap.set(item.placeId, item.imagePath);
        }
      }

      imageMetadataCache = lookupMap;
      console.log(`✅ Loaded ${lookupMap.size} generated images (v2 only)`);
    } catch (error) {
      console.error('❌ Error loading image metadata:', error);
      // Set empty map on error so we don't retry constantly
...
/**
 * Get generated image path for a placeId (only _v2 images)
 * Returns null if no generated image exists
 */
async function getGeneratedImagePath(placeId: string): Promise<string | null> {
  const metadata = await loadImageMetadata();
  return metadata.get(placeId) || null;
}

/**
 * Determine which city dataset to use based on coordinates
 * Maps coordinates to available city datasets
 */
function determineCityFromCoordinates(lat: number, lng: number): string {
  // City coordinate ranges (approximate centers)
  const cityRanges: Array<{ city: string; latMin: number; latMax: number; lngMin: number; lngMax: number }> = [
    { city: 'bangkok', latMin: 13.5, latMax: 14.0, lngMin: 100.3, lngMax: 100.8 },
    { city: 'barcelona', latMin: 41.2, latMax: 41.5, lngMin: 2.0, lngMax: 2.3 },
    { city: 'hongkong', latMin: 22.1, latMax: 22.5, lngMin: 113.8, lngMax: 114.4 },
    { city: 'mumbai', latMin: 18.9, latMax: 19.2, lngMin: 72.7, lngMax: 73.0 },
    { city: 'singapore', latMin: 1.2, latMax: 1.5, lngMin: 103.6, lngMax: 104.0 },
    { city: 'mexicocity', latMin: 19.2, latMax: 19.6, lngMin: -99.3, lngMax: -98.9 },
    { city: 'london', latMin: 51.3, latMax: 51.7, lngMin: -0.5, lngMax: 0.2 },
    { city: 'tokyo', latMin: 35.5, latMax: 35.8, lngMin: 139.5, lngMax: 139.9 },
    { city: 'paris', latMin: 48.7, latMax: 48.9, lngMin: 2.2, lngMax: 2.5 },
    { city: 'newyork', latMin: 40.6, latMax: 40.9, lngMin: -74.1, lngMax: -73.7 },
  ];
  ...
  for (const range of cityRanges) {
    if (
      ...
      console.log(`📍 Detected city: ${range.city} for coordinates (${lat}, ${lng})`);
      return range.city;
    }
  }

  // Country-level fallbacks for regions without specific city matches
  // India (broad range): default to Mumbai
  if (lat >= 8.0 && lat <= 37.0 && lng >= 68.0 && lng <= 97.0) {
    console.log(`📍 Coordinates (${lat}, ${lng}) in India, defaulting to mumbai`);
    return 'mumbai';
  }

  // Thailand (broader range): default to Bangkok
  if (lat >= 5.0 && lat <= 20.0 && lng >= 97.0 && lng <= 106.0) {
    console.log(`📍 Coordinates (${lat}, ${lng}) in Thailand, defaulting to bangkok`);
    return 'bangkok';
  }

  // Default to barcelona if no match (fallback)
  console.log(`📍 No city match for coordinates (${lat}, ${lng}), defaulting to barcelona`);
  return 'barcelona';
}
 ...
 * Now checks for generated OpenAI images (_v2) before falling back
 */
export const transformMasterSetToFeedCard = async (
  ...
  // Check for generated OpenAI image (_v2) first
  let imageUrl: string;
  try {
    const generatedImagePath = await getGeneratedImagePath(location.placeId);
    if (generatedImagePath) {
      imageUrl = generatedImagePath;
      console.log(`✅ Using generated image for ${location.placeId}: ${generatedImagePath}`);
    } else {
      // Fallback to cuisine-based image
      imageUrl = generateRestaurantFallbackImage(location.categoryName);
    }
  } catch (error) {
    console.warn(`⚠️ Error checking generated image for ${location.placeId}, using fallback:`, error);
    imageUrl = generateRestaurantFallbackImage(location.categoryName);
  }
...
export const FeedService = {
  ...
  async fetchRestaurantCardsFromJSON(
    ...
    try {
      ...
      // Determine city from user location or default
      const city = userLocation
        ? determineCityFromCoordinates(userLocation.lat, userLocation.lng)
        : 'barcelona'; // Default if no location

      // City center coordinates for each city (used when user is far from city)
      ...
      if (userLocation) {
        // Check if user is actually in the detected city or if we defaulted
        const cityCenter = cityCenters[city];
        if (cityCenter) {
          // Calculate distance from user to city center
          ...
          // If user is more than 50km from city center, use random locations instead
          // This handles cases where we default to a city (e.g., India -> Mumbai)
          if (distanceToCityCenter > 50) {
            console.log(`📍 User is ${distanceToCityCenter.toFixed(1)}km from ${city} center, using random locations`);
            locations = await LocationDataService.getRandomLocations(count * 3, city);
          } else {
            // User is close to city, use nearby search
            ...
            // If nearby search returns few results, fallback to random
            if (nearbyWithDistance.length < count) {
              console.log(`📍 Only ${nearbyWithDistance.length} nearby locations found, supplementing with random`);
              const randomLocations = await LocationDataService.getRandomLocations(count * 2, city);
              // Combine and deduplicate by placeId
              const allLocations = [...nearbyWithDistance];
          ...
          // City center not found, use random
          locations = await LocationDataService.getRandomLocations(count * 3, city);
        }
      } else {
      ...
      // Transform to RestaurantCard format (now async)
      const cards = await Promise.all(
        locations.map((loc) => transformMasterSetToFeedCard(loc, userLocation))
      );
  ...
  async fetchRestaurantCards(
    ...
    try {
      ...
      // If we don't have enough unique restaurants, that's okay - return what we have
      // The primary source (fetchRestaurantCardsFromJSON) uses local JSON data which has comprehensive coverage
      if (filteredRestaurants.length < count) {
        console.log(`⚠️ Only ${filteredRestaurants.length} unique restaurants found (requested ${count})`);
        console.log('💡 Tip: The app primarily uses local JSON data which has better coverage. This fallback API path is rarely used.');
      }
