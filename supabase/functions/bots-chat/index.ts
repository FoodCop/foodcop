...
// Send bot reply via Supabase Realtime (replaces Stream Chat)
...
// Main Edge Function - now handles direct bot chat requests instead of webhooks
Deno.serve(async (req) => {
  try {
    // Parse request body
    const { channelId, botId, message, userId } = await req.json();

    if (!channelId || !botId || !message) {
    ...
    if (topic === "unsafe") {
      ...
      reply = deflectAndPivot(
        ...
        bot.specialties || [],
        text,
        undefined // No user country available in this context
      );
    } else {
    ...
    const messageSent = await sendRealtimeMessage(channelId, botId, reply);
