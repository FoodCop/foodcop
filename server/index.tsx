...
try {
  if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
    ...
    console.warn('⚠️ Supabase service role credentials missing - privileged endpoints will be disabled');
  }
  if (SUPABASE_URL && SUPABASE_ANON_KEY) {
    ...
    console.warn('⚠️ Supabase anon credentials missing - public endpoints will be disabled');
  }
} catch (error) {
...
app.post("/make-server-5976446e/auth/signup", async (c) => {
  ...
  return c.json({
    message: 'Use Supabase client-side auth.signUp({ email, password }) from the browser. Then PATCH /auth/profile to create profile.'
  });
});
...
app.patch("/make-server-5976446e/auth/profile", async (c) => {
  try {
    ...
    const updates = {
      ...parsed.data,
      dietary_preferences: parsed.data.dietary_preferences ? JSON.stringify(parsed.data.dietary_preferences) : undefined,
      cuisine_preferences: parsed.data.cuisine_preferences ? JSON.stringify(parsed.data.cuisine_preferences) : undefined
    };
    ...
    return c.json({ success: true, message: 'Profile updated', profile: updates });
  } catch (err) {
...
app.get("/make-server-5976446e/auth/profile", async (c) => {
  try {
    ...
    let profileRow = null;
    try {
      const { data: row, error: rowError } = await supabaseAnon.from('users').select('*').eq('id', user.id).maybeSingle();
      if (rowError) {
    ...
    if (!profileRow) {
      ...
      try {
        ...
        if (insertError) console.warn('⚠️ Failed to create placeholder profile:', insertError.message);
    ...
    const mergedProfile = {
      ...
      display_name: profileRow.display_name || user.user_metadata?.full_name || user.email?.split('@')[0] || 'FUZO User',
      bio: profileRow.bio || '',
...
app.get("/make-server-5976446e/profile/saved-restaurants", async (c) => {
  try {
    ...
    const savedData = await kv.get(savedRestaurantsKey) || { restaurants: [] };
...
app.post("/make-server-5976446e/profile/save-restaurant", async (c) => {
  try {
    ...
    const savedRestaurantsKey = `user_saved_restaurants_${user.id}`;
    const existingSaved = await kv.get(savedRestaurantsKey) || { restaurants: [] };
    
    // Check if already saved
    const isAlreadySaved = existingSaved.restaurants.some((r: any) => 
      r.place_id === restaurant.place_id || r.id === restaurant.id
    );
    ...
    const savedRestaurant = {
      ...
      cuisine: Array.isArray(restaurant.cuisine) ? restaurant.cuisine.join(', ') : (restaurant.cuisine || 'Restaurant'),
      price: '$'.repeat(restaurant.price_level || restaurant.priceLevel || 1),
      location: restaurant.vicinity || restaurant.formatted_address || restaurant.address || 'Unknown location',
...
app.delete("/make-server-5976446e/profile/unsave-restaurant/:placeId", async (c) => {
  try {
    ...
    existingSaved.restaurants = existingSaved.restaurants.filter((r: any) => r.place_id !== placeId);
    existingSaved.lastUpdated = new Date().toISOString();
