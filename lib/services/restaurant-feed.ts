...
/**
 * Restaurant Feed Service
 * Handles fetching and managing restaurant recommendations for the feed
 */
export class RestaurantFeedService {
  
  /**
   * Get restaurant feed posts from Master Bots
  ...
  /**
   * Like a Master Bot post (engagement tracking)
   */
  ...
  /**
   * Get Master Bot feed statistics
   */
  static async getBotFeedStats(botUsername: string) {
    try {
      const supabase = await supabaseServer();
      ...
      const posts = data || [];
      const totalLikes = posts.reduce((sum: number, p: any) => sum + (p.engagement_likes || 0), 0);
      const totalComments = posts.reduce((sum: number, p: any) => sum + (p.engagement_comments || 0), 0);
      const avgRating = posts.length > 0 
        ? posts.reduce((sum: number, p: any) => sum + parseFloat(p.restaurant_rating || '0'), 0) / posts.length 
        : 0;

      // Get top cuisines
      const cuisineCounts: Record<string, number> = {};
      posts.forEach((p: any) => {
        if (p.restaurant_cuisine) {
          cuisineCounts[p.restaurant_cuisine] = (cuisineCounts[p.restaurant_cuisine] || 0) + 1;
        }
      });
      ...
      return {
        ...
        total_saves: totalComments, // Using comments as proxy for saves
        avg_rating: avgRating,
        top_cuisines: topCuisines,
        recent_activity: posts.length > 0 ? posts[0].created_at : null
      };

    } catch (error) {
      console.error('Failed to get bot feed stats:', error);
      return null;
    }
  }
  ...
  /**
   * Filter restaurants by location and other criteria
   */
